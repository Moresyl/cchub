...
function ProxyAdvanced() {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await invoke("set_optimizer_config", { config });
      if (rectConfig) {
        await invoke("set_rectifier_config", { config: rectConfig });
      }
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        {rectConfig && (
          <div>
            ...
            />
            <div style={{ marginTop: 8, paddingLeft: 28, display: "flex", flexDirection: "column", gap: 10, opacity: rectConfig.enabled ? 1 : 0.5, pointerEvents: rectConfig.enabled ? "auto" : "none" }}>
              <ToggleRow
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 6 }}>
                  {config.bodyFilterWhitelist.map((item, idx) => (
                    <span key={idx} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 8px", borderRadius: 12, background: "var(--bg-app)", border: "1px solid var(--border-default)", fontSize: 11 }}>
                      {item}
                      <button
                        style={{ border: "none", background: "none", cursor: "pointer", padding: 0, color: "var(--text-muted)", fontSize: 11 }}
                        onClick={() => update({ bodyFilterWhitelist: config.bodyFilterWhitelist.filter((_, i) => i !== idx) })}
                      >
                        ×
                      </button>
                    </span>
              ...
              <div style={{ marginTop: 8, paddingLeft: 28, display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div>
                  ...
                  <button
                    ...
                    onClick={() => update({ modelMapperRules: [...config.modelMapperRules, { from: "", to: "", matchMode: "contains" }] })}
                  >
                    <Plus size={12} />

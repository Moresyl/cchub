...
界面采用紧凑的明暗中性主题，使用统一的无边框桌面顶栏、原生窗口操作、折叠侧栏与 `Ctrl+K` 快速应用配置。新配置仅提供官方默认值或空白自定义模板，已有配置档案继续兼容。配置文件、MCP 服务和 Skills 位于辅助导航中。此前的 Autopilot、会话统计、Marketplace、安全审计等独立页面不再作为当前产品入口；升级不会删除已有配置数据。

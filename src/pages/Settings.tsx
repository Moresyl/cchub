...
export default function Settings() {
  ...
  const i = t();
  const loc = getLocale();
  const setLocalProviderProxySettingsMutation = useSetLocalProviderProxySettingsMutation();
  const setWindowPreferencesMutation = useSetWindowPreferencesMutation();
  const setVisibleAppsMutation = useSetVisibleAppsMutation();
  const setPreferredTerminalMutation = useSetPreferredTerminalMutation();
  const setLogPreferencesMutation = useSetLogPreferencesMutation();
  const setSkillSyncMethodMutation = useSetSkillSyncMethodMutation();
  const saveCustomPathMutation = useSaveCustomPathMutation();
  const setProxyMutation = useSetProxyMutation();
  ...
  useEffect(() => {
    ...
    getVersion()
      ...
      .catch((error) => console.warn("Failed to load app version", error));
  }, []);
  ...
  async function loadSkillSyncMethod() {
    try {
      ...
      console.warn("Failed to load skill sync method", error);
    }
  }
  ...
  const handleSaveLocalProviderProxySettings = useCallback(
    async (next: LocalProviderProxySettings) => {
      setSavingLocalProviderProxy(true);
      try {
        const saved = await setLocalProviderProxySettingsMutation.mutateAsync({ settings: next });
        ...
        showToast(
          ...
          uiText(
            "本地 Provider 代理设置已保存",
            "Local provider proxy settings saved",
            "ローカル Provider プロキシ設定を保存しました",
          ),
        );
      } catch (e) {
    ...
    [loc, setLocalProviderProxySettingsMutation],
  );
  ...
  const updateWindowPreference = useCallback(
    async <K extends keyof WindowPreferences>(key: K, value: WindowPreferences[K]) => {
      const nextPreferences = { ...windowPreferences, [key]: value };
      setSavingWindowKey(key);
      try {
        const saved = await setWindowPreferencesMutation.mutateAsync({ preferences: nextPreferences });
    ...
    [loc, setWindowPreferencesMutation, windowPreferences],
  );

  const handleToggleVisibleApp = useCallback(
    async (appId: ManagedAppId) => {
      ...
      try {
        const saved = normalizeVisibleApps(await setVisibleAppsMutation.mutateAsync({ visibleApps: nextVisibleApps }));
    ...
    [loc, setVisibleAppsMutation, visibleApps],
  );
  ...
  const handleSelectTerminal = useCallback(
    async (terminalId: string) => {
      ...
      try {
        const nextPreferences = await setPreferredTerminalMutation.mutateAsync({ terminalId });
    ...
    [loc, setPreferredTerminalMutation],
  );
  ...
  const handleSaveLogLevel = useCallback(
    async (level: string) => {
      ...
      try {
        const saved = await setLogPreferencesMutation.mutateAsync({ preferences: { level } });
    ...
    [loc, setLogPreferencesMutation],
  );

  const handleSyncMethodChange = useCallback(
    async (method: "symlink" | "copy") => {
      try {
        await setSkillSyncMethodMutation.mutateAsync({ method });
    ...
    [loc, setSkillSyncMethodMutation],
  );
  ...
  const openInSystemWithLabel = useCallback(
    async (target: string, label: string) => {
      try {
        await invoke("open_in_system", { target });
      } catch (e) {
        showToast("error", `${i.settings.openFailed.replace("{label}", label)}: ${String(e)}`);
      }
    },
  ...
  const saveToolCustomPath = useCallback(
    async (toolId: string, configDir: string | null, mcpConfigPath: string | null, skillsDir: string | null) => {
      await saveCustomPathMutation.mutateAsync({ toolId, configDir, mcpConfigPath, skillsDir });
  ...
  const persistProxy = useCallback(
    async (successMessage: string) => {
      try {
        await setProxyMutation.mutateAsync({ proxyUrl });
        markProxySaved();
    ...
    [markProxySaved, proxyUrl, setProxyMutation],
  );
  ...
  const handleProxyInputKeyDown = useCallback(
  ...
  const handleSaveProxy = useCallback(() => {
    void persistProxy(
      loc === "zh" ? (proxyUrl.trim() ? "代理已设置" : "代理已清除") : proxyUrl.trim() ? "Proxy set" : "Proxy cleared",
    );

...
const WIZARD_PRESETS: WizardPreset[] = [
  { id: "npx", labelZh: "Node / npx", labelEn: "Node / npx", command: "npx", args: ["-y", "@modelcontextprotocol/server-example"] },
...
export default function McpServers() {
  ...
  const handleWizardInstall = useCallback(async () => {
    ...
    try {
      const created = await invoke<McpServer>("install_mcp_server", {
      ...
      {wizardOpen && (
        <div className="section-card" style={{ marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
            ...
            <button className="btn btn-ghost btn-icon-sm" onClick={closeWizard}><X size={14} /></button>
          </div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
            {[
              ...
              <div key={label} className={`badge ${wizardStep === index + 1 ? "badge-accent" : "badge-muted"}`} style={{ padding: "6px 10px" }}>
                {label}
              </div>
            ))}
          </div>
          ...
          {wizardStep === 1 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div>
                <div className="field-label" style={{ marginBottom: 8 }}>{zh ? "常用模板" : "Quick Templates"}</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 14 }}>
                <div>
                  ...
                  <textarea
                    ...
                    style={{ minHeight: 118, resize: "vertical", fontFamily: "'JetBrains Mono', monospace", fontSize: 12, paddingTop: 10 }}
                  />
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={zh ? "每行 KEY=value，或直接粘贴 JSON 对象" : "Use KEY=value per line, or paste a JSON object"}
                    style={{ minHeight: 118, resize: "vertical", fontFamily: "'JetBrains Mono', monospace", fontSize: 12, paddingTop: 10 }}
                  />
                </div>
              </div>
                    ...
                    return (
                      <label key={tool.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={() => setWizardSyncTargets((current) => (
                            checked
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {(wizardValidation.errors.length > 0 || wizardValidation.warnings.length > 0) && (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {wizardValidation.errors.map((message) => (
                    <div key={`wizard-error:${message}`} className="card" style={{ padding: "10px 12px", borderColor: "var(--danger)", background: "color-mix(in srgb, var(--danger) 8%, var(--bg-card))", fontSize: 12 }}>
                      {zh ? `错误：${message}` : `Error: ${message}`}
                    </div>
                  ))}
                  {wizardValidation.warnings.map((message) => (
                    <div key={`wizard-warning:${message}`} className="card" style={{ padding: "10px 12px", borderColor: "var(--warning)", background: "color-mix(in srgb, var(--warning) 10%, var(--bg-card))", fontSize: 12 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 14 }}>
                <div>
                  ...
                  <CodeEditor value={JSON.stringify(wizardValidation.parsedArgs, null, 2)} language="json" readOnly minHeight={100} maxHeight={180} />
                </div>
                <div>
                  <div className="field-label">{zh ? "环境变量解析结果" : "Parsed Environment"}</div>
                  <CodeEditor value={JSON.stringify(wizardValidation.parsedEnv, null, 2)} language="json" readOnly minHeight={100} maxHeight={180} />
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 18, gap: 8, flexWrap: "wrap" }}>
            <div>
              {wizardStep > 1 && (
                <button className="btn btn-secondary btn-sm" onClick={() => setWizardStep((current) => Math.max(1, current - 1))} style={{ gap: 6 }}>
                  <ArrowLeft size={14} />
                  {zh ? "上一步" : "Back"}
                </button>
              )}
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              {wizardStep < 3 ? (
                ...
                <button
                  ...
                  style={{ gap: 6 }}
                >
                  {wizardInstalling ? <div className="spinner" style={{ width: 12, height: 12 }} /> : <PackagePlus size={14} />}

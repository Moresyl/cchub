...
function SettingsLocalProviderProxySectionComponent({
  ...
  const enabledApps = settings.enabled_apps.filter((appId): appId is ManagedAppId =>
    MANAGED_APPS.includes(appId as ManagedAppId),
  );
  ...
  return (
    <div className="section-card">
      ...
      <p style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 14 }}>
        {uiText(
          locale,
          "把 Claude / Codex / Gemini / Grok Build / OpenCode / OpenClaw / Hermes 的活动 Provider 改写到本机回环地址，由 CCHub 在请求时动态转发到当前选中的 Provider。切换 Provider 时无需重启代理服务。",
        ...
        <div style={{ padding: "12px 14px", borderRadius: 10, background: "var(--bg-input)" }}>
          ...
          <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 8 }}>
            {status?.running
              ...
              : uiText(
                  locale,
                  "当前未运行，至少启用一个 App 后保存即可启动",
                  "Not running. Enable at least one app and save to start it.",
                  "現在は停止中です。少なくとも 1 つの App を有効化して保存すると起動します。",
                )}
          </div>
        </div>
      ...
      <p style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 14 }}>
        {uiText(
          locale,
          "当前版本已具备本地代理接管、按 App 开关、热切换、格式转换、候选端点自动重试、故障转移队列与成本统计。高级代理设置中还可以查看熔断状态并手动恢复。",
          "This revision includes local proxy takeover, per-app toggles, hot switching, format conversion, endpoint retries, a persistent failover queue, and cost accounting. Advanced proxy settings also expose circuit status and manual recovery.",
          "この版ではローカルプロキシ接管、App 単位の切替、ホットスイッチ、形式変換、候補エンドポイント再試行、永続フェイルオーバーキュー、コスト計算を利用できます。詳細設定ではサーキット状態の確認と手動復旧も可能です。",
        )}
      </p>

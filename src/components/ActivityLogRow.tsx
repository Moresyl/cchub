...
function ActivityLogRowComponent({ item, recordedAtLabel }: ActivityLogRowProps) {
  return (
    <div className="list-row cv-auto" style={{ padding: "10px 12px", containIntrinsicSize: "0 40px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
        <span
          className={`dot ${item.status === "success" ? "dot-active" : item.status === "error" ? "dot-error" : "dot-disabled"}`}
        />
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
        ...
        <span style={{ fontSize: 11, color: "var(--text-muted)" }}>{recordedAtLabel}</span>
      </div>
    </div>
  );
}

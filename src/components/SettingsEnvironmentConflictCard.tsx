...
function SettingsEnvironmentConflictCardComponent({ conflict, locale }: SettingsEnvironmentConflictCardProps) {
  ...
  const title =
    conflict.kind === "multi_tool_override"
      ? uiText(
          locale,
          "检测到多套 CLI 环境覆盖",
          "Multiple CLI override groups detected",
          "複数の CLI 上書きグループを検出しました",
        )
      : uiText(
          locale,
    ...
    <div style={{ padding: "12px 14px", borderRadius: 10, background: "var(--bg-input)" }}>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        {conflict.variables.map((item) => (
          <code key={item} className="badge badge-accent" style={{ fontSize: 11, fontFamily: "var(--font-code)" }}>
            {item}
          </code>
        ))}
      </div>

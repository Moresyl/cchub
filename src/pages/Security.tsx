...
import { getLocale, t, tReplace } from "../lib/i18n";
...
export default function Security() {
  ...
  const getRiskBadge = useCallback(
    (risk: string) => {
      switch (risk) {
        case "high":
          return <span className="badge badge-danger">{i.security.riskHigh}</span>;
        case "medium":
          return <span className="badge badge-warning">{i.security.riskMedium}</span>;
        default:
          return <span className="badge badge-success">{i.security.riskLow}</span>;
      }
    },
  ...
  if (loading && !scanned) {
    return <LoadingState label={i.security.scanning} />;
  }
  ...
  return (
    <div className="animate-in" style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      <div className="page-header">
        ...
        >
          <RefreshCw size={14} />
          {loading ? i.security.scanning : i.security.runAudit}
        </button>
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: "auto" }}>
        {auditError ? (
          <ErrorState
            title={uiText("安全审计失败", "Security audit failed", "セキュリティ監査に失敗しました")}
            message={auditError}
            retryLabel={i.security.runAudit}
            onRetry={() => {
              void runAudit();
            }}
          />
          ...
          <EmptyState
            title={i.security.noIssues}
            description={i.security.noIssuesTip}
            icon={<Shield size={28} style={{ color: "var(--text-muted)" }} />}
          />
        ) : (
